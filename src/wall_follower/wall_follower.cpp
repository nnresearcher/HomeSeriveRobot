...
bool robot_move(const ROBOT_MOVEMENT move_type)
{
    if (move_type == STOP) {
        ...
        motor_command.angular.z = 0.0;
        motor_command.linear.x = 0.5*rate_sudo;
    }

    else if (move_type == BACKWARD) {
        ROS_INFO("[ROBOT] I'm going back! \n");
        motor_command.linear.x = -0.75*rate_sudo;
        motor_command.angular.z = 0.0;
    }
        ...
        motor_command.linear.x = 0.0;
        motor_command.angular.z = 1.0*rate_sudo;
    }
        ...
        motor_command.linear.x = 0.25*rate_sudo;
        motor_command.angular.z = -0.25*rate_sudo;
    }
...
int main(int argc, char** argv)
{
    ...
    motor_command_publisher = nodehandle.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/navi", 100);

    // Subscribe to the /scan topic and call the laser_callback function
    laser_subscriber = nodehandle.subscribe("/scan", 1000, laser_callback);
